import Web3 from "web3";

import { ChainId, SupportedChainId } from "core/constans/chains";

import { ENV } from "../../env";

type Chains = {
  [K in ChainId]: {
    readonly provider: string;
    readonly soulboundContract: string;
  };
};

export const web3Factory = (
  blockchainId: ChainId,
): { web3: Web3; soulboundContract: string } => {
  const supportedChains: Chains = {
    [SupportedChainId.POLYGON]: {
      provider: ENV.VITE_APP_POLYGON_ALCHEMY_URL,
      soulboundContract: ENV.VITE_APP_POLYGON_SOULBOUND_CONTRACT,
    },
    [SupportedChainId.SEPOLIA]: {
      provider: ENV.VITE_APP_SEPOLIA_ALCHEMY_URL,
      soulboundContract: ENV.VITE_APP_SEPOLIA_SOULBOUND_CONTRACT,
    },
    [SupportedChainId.POLYGON_MUMBAI]: {
      provider: ENV.VITE_APP_MUMBAI_INFURA_URL,
      soulboundContract: ENV.VITE_APP_MUMBAI_SOULBOUND_CONTRACT,
    },
    // [SupportedChainId.SCROLL_ALPHA]: {
    //   provider: "https://alpha-rpc.scroll.io/l2",
    //   soulboundContract: ENV.VITE_APP_SCROLL_SOULBOUND_CONTRACT,
    // },
    [SupportedChainId.SCROLL_SEPOLIA]: {
      provider: "https://sepolia-rpc.scroll.io",
      soulboundContract: ENV.VITE_APP_SCROLL_SEPOLIA_SOULBOUND_CONTRACT,
    },
    [SupportedChainId.BNB]: {
      provider: "https://bsc-dataseed.bnbchain.org",
      soulboundContract: ENV.VITE_APP_BNB_SOULBOUND_CONTRACT,
    },
    [SupportedChainId.BNB_TESTNET]: {
      provider: "https://bsc-testnet-dataseed.bnbchain.org",
      soulboundContract: ENV.VITE_APP_BNB_TESTNET_SOULBOUND_CONTRACT,
    },
    [SupportedChainId.SKALE_NEBULA_TESTNET]: {
      provider:
        "https://staging-v3.skalenodes.com/v1/staging-faint-slimy-achird",
      soulboundContract: ENV.REACT_APP_SKALE_NEBULA_TESTNET_SOULBOUND_CONTRACT,
    },
    [SupportedChainId.SCROLL_MAINNET]: {
      provider: "https://rpc.scroll.io/",
      soulboundContract: ENV.REACT_APP_SCROLL_MAINNET_SOULBOUND_CONTRACT,
    },
  };

  const web3Instance = supportedChains[blockchainId];

  const web3 = new Web3(web3Instance.provider);

  const soulboundContract = web3Instance.soulboundContract;

  return { web3, soulboundContract };
};
